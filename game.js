...
const state = {
  ...
  score: 0, level: 1, xp: 0, nextXp: 100, core: 100, fireCooldown: 0,
  ...
  upgrades: { damage: 18, fireRate: 240, speed: 2.7 }
};
...
function updateUI() {
  ...
  setText('killsValue', state.totalKills); setText('scoreValue', String(state.score).padStart(5, '0')); setText('coreValue', `${Math.max(0, Math.round(state.core))}%`);
  ...
  $('waveBar').style.width = `${Math.min(100, state.waveKills / (4 + state.wave * 2) * 100)}%`; setText('sentryValue', `${state.towers.length} / 4`); setText('towerValue', `${4 - state.towers.length} CHARGES READY`);
...
function reset() {
  Object.assign(state, { running: true, gameOver: false, time: 0, wave: 1, waveKills: 0, totalKills: 0, score: 0, level: 1, xp: 0, nextXp: 100, core: 100, fireCooldown: 0, spawnTimer: 0, dashCooldown: 0, buildMode: false, bossSpawned: false, bullets: [], enemies: [], towers: [], particles: [], upgrades: { damage: 18, fireRate: 240, speed: 2.7 } });
...
function showUpgrades() {
  const choices = [['OVERCHARGE', '+8 weapon damage', () => state.upgrades.damage += 8], ['QUICK HANDS', '+60 RPM fire rate', () => state.upgrades.fireRate += 60], ['LIGHT FEET', '+0.55 movement speed', () => state.upgrades.speed += .55], ['NANITE PATCH', '+15 core integrity', () => state.core = Math.min(100, state.core + 15)]];
...
function shoot(x = state.player.x, y = state.player.y, angle = state.player.angle, damage = state.upgrades.damage, source = 'player') { state.bullets.push({ x: x + Math.cos(angle) * 18, y: y + Math.sin(angle) * 18, vx: Math.cos(angle) * 8, vy: Math.sin(angle) * 8, life: 1.1, damage, source }); sound('shot'); }
function buildTower(x, y) { if (state.towers.length >= 4 || Math.hypot(x - state.player.x, y - state.player.y) < 65) return; state.towers.push({ x, y, cooldown: 0, angle: 0 }); state.buildMode = false; sound('build'); log('Sentry deployed. Grid is expanding.'); updateUI(); }
function towerFire(tower, target) { tower.angle = Math.atan2(target.y - tower.y, target.x - tower.x); shoot(tower.x, tower.y, tower.angle, state.upgrades.damage * .55, 'tower'); tower.cooldown = .8; }
function update(dt) {
  ...
  if (state.keys[' '] && state.dashCooldown <= 0 && (dx || dy)) { p.x += dx * 70; p.y += dy * 70; state.dashCooldown = 2; burst(p.x, p.y, '#c7f36b', 10); }
  ...
  state.towers.forEach(tower => { tower.cooldown -= dt; if (tower.cooldown <= 0) { const target = state.enemies.filter(e => e.type !== 'boss' || state.waveKills > 2).sort((a, b) => Math.hypot(a.x - tower.x, a.y - tower.y) - Math.hypot(b.x - tower.x, b.y - tower.y))[0]; if (target && Math.hypot(target.x - tower.x, target.y - tower.y) < 260) towerFire(tower, target); } });
...
function draw() {
  ...
  state.towers.forEach(t => { ctx.save(); ctx.translate(t.x, t.y); ctx.rotate(t.angle); ctx.fillStyle = '#c7f36b'; ctx.shadowBlur = 14; ctx.shadowColor = '#c7f36b'; ctx.fillRect(-12, -12, 24, 24); ctx.fillStyle = '#0b1411'; ctx.fillRect(0, -3, 19, 6); ctx.restore(); });
...
window.addEventListener('keydown', event => { state.keys[event.key.toLowerCase()] = true; if (event.key === ' ') event.preventDefault(); if (event.key.toLowerCase() === 't') state.buildMode = !state.buildMode; }); window.addEventListener('keyup', event => state.keys[event.key.toLowerCase()] = false);
...
const towerButton = $('towerButton'); towerButton.onclick = () => { state.buildMode = !state.buildMode; towerButton.classList.toggle('selected', state.buildMode); setText('upgradeNote', state.buildMode ? 'Click the arena to place a sentry.' : 'Clear hostiles to earn XP.'); };
